...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        return http
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(PathRequest.toStaticResources().atCommonLocations()).permitAll() //정적 리소스에 요청에 대한 권한 설정
                        .mvcMatchers(HttpMethod.POST, "/**").hasAnyRole(rolesAboveManager) //POST(수정)에 대한 모든 요청에 대해, 위의 역할 중 하나를 가지고 있는 사용자에게 권한 설정
                        .mvcMatchers(HttpMethod.DELETE, "/**").hasAnyRole(rolesAboveManager) //DELETE(삭제)에 대한 모든 요청에 대해, 위의 역할 중 하나를 가지고 있는 사용자에게 권한 설정
                        .anyRequest().authenticated() //모든 요청이 인증된 사용자에게만 허용 == 로그인한 사용자에게만 허용
                )
    ...
    public OAuth2UserService<OAuth2UserRequest, OAuth2User> oAuth2UserService(
        ...
        return userRequest -> {
            ...
            Set<RoleType> roleTypes = Set.of(RoleType.USER); //최초 가입시에는 최하의 권한인 "USER"로 설정을 해준다.
